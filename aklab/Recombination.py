...
class Recombination(object):
    def __init__(self,data,mass=2,p0=None,tag="",fit_range=None,jf=None,disp=True,tlim=1000,kd=5.1e-31,fix_jf=True,k1=8.286246100559756,k2=69.68,c1=3.0385593350848646e-07,style=False,yoffset=None):
        ...
        if fit_range:
            if type(fit_range) == list:
                self.fit_data = self.data[np.abs(np.array(self.data['tsec']) - fit_range[0]).argmin():np.abs(np.array(self.data['tsec']) - fit_range[1]).argmin()]
            else:
        ...
        if disp:
            ...
            if fix_jf:
                l2 = self.plot(np.linspace(0,tlim,tlim*100),self.fitting_function(np.linspace(0,tlim,tlim*100),self.param[0],self.param[1]),label="fitting",color='r',fig=False,convert=False)
            else:
                l2 = self.plot(np.linspace(0,tlim,tlim*100),self.fitting_function2(np.linspace(0,tlim,tlim*100),self.param[0]),label="fitting",color='r',fig=False,convert=False)
            if style:
                a = r"${\rm J}{\ }{\rm (\times 10^"
                b = f"{yoffset}"
                c = r"m^{-2}s^{-1})}$"
    ...
    def fitting_function(self,t,a,b):
        at = np.exp(t*a) * (np.sqrt(self.jf+b) + np.sqrt(self.ji)) / (np.sqrt(self.jf+b) - np.sqrt(self.ji))
    ...
    def plot(self,x,y,label="",title=None,scatter=False,color=None,fig=True,convert=True,co=[8.733935665396627,69.68,2.978590280897269e-07],subtract=False):
        font_setup(size=28)
        if fig:
