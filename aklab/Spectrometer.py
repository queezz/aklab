...
class Spectrometer:
    ...
    def display_nc(self,fp,xlim=None,ylim=None,disp=True,subplot=None,fig=None,style=False,yoffset=None,title=None,line=None):
        ...
        if disp:
            if subplot:
                ax = fig.add_subplot(subplot[0],subplot[1],subplot[2], xlabel=r"${\rm Wavelength}{\ }{\rm (nm)}$")
            else:    
    ...
    def display_gaussian(self, data, out, rangeis, xlimit = None, ylimit = None, num_gauss = 1,subplot=None,fig=None,fontsize=28):
        ...
        if subplot:
            ax = fig.add_subplot(subplot[0],subplot[1],subplot[2], xlabel=r"${\rm Wavelength}{\ }{\rm (nm)}$", ylabel=r"${\rm Intensity}{\ }{\rm (arb.unit)}$")
        else:
            fig = plt.figure(figsize=(16, 9), dpi=50)
            ax = fig.add_subplot(1,1,1, xlabel=r"${\rm Wavelength}{\ }{\rm (nm)}$", ylabel=r"${\rm Intensity}{\ }{\rm (arb.unit)}$")
        ax.plot(data[1], data[0] - out.best_values["c"], 'o', label=r"Data",c='k')
        if num_gauss == 1:
            ax.plot(x2, self.gaussian(x2, out.best_values["amplitude"], out.best_values["center"], out.best_values["sigma"]), '-', label=r'Fitting',c='r')
        elif num_gauss == 2:
            ax.plot(x2, self.gaussian(x2, out.best_values["gauss1_amplitude"], out.best_values["gauss1_center"], out.best_values["gauss1_sigma"]), '-', label=r'Low temperature',c='C1')
            ax.plot(x2, self.gaussian(x2, out.best_values["gauss2_amplitude"], out.best_values["gauss2_center"], out.best_values["gauss2_sigma"]), '-', label=r'High temperature',c='C2')
            ax.plot(x2, self.gaussian(x2, out.best_values["gauss1_amplitude"], out.best_values["gauss1_center"], out.best_values["gauss1_sigma"]) + self.gaussian(x2, out.best_values["gauss2_amplitude"], out.best_values["gauss2_center"], out.best_values["gauss2_sigma"]), '-', label=r'Summation',c='r')
    ...
    def calc_fwhm(self, fp, rangeis=None, xlimit = None, ylimit = 0, num_gauss = 1,disp=True,subplot=None,fig=None,fontsize=28):
        if type(fp) == str:#when row nc data is selected
            data = self.convert_nc(fp)
        elif type(fp) != np.ndarray:
            return 'please insert fp="row nc data" or data="converted nc data"'
        ...
        if disp:
            self.display_gaussian(data, out, rangeis=rangeis, xlimit=xlimit, ylimit=ylimit, num_gauss=num_gauss,subplot=subplot,fig=fig,fontsize=fontsize)
        if num_gauss == 1:
            ...
            self.sigma = [[out.params['gauss1_sigma'].value,out.params['gauss1_sigma'].stderr],[out.params['gauss2_sigma'].value,out.params['gauss2_sigma'].stderr]]
            fwhm = 2 * np.array(self.sigma) * np.sqrt(2 * np.log(2))
            print(f"FWHM {fwhm[0][0]:.4f} +- {fwhm[0][1]:.4f} nm and {fwhm[1][0]:.4f} +- {fwhm[1][1]:.4f} nm intencity {intencity}   ", end="")
        return  fwhm
    ...
    def calc_temperature(self,fwhm1,fwhm2,explain="",A=1.0079):
        if len(fwhm1)==1:
            ...
            atom_temperature = (deltalam / (7.16e-7 * self.wavelength))**2 * A
            print(f'{atom_temperature[0]:.2f} +{atom_temperature[1]-atom_temperature[0]} -{atom_temperature[0]-atom_temperature[2]}  K   {explain}')
        else:
            fwhm = np.array([[fwhm1[0][0],fwhm1[0][0]+fwhm1[0][1],fwhm1[0][0]-fwhm1[0][1]],[fwhm1[1][0],fwhm1[1][0]+fwhm1[1][1],fwhm1[1][0]-fwhm1[1][1]]])
            deltalam = (fwhm**2-fwhm2[0][0]**2)**0.5
            atom_temperature = (deltalam / (7.16e-7 * self.wavelength))**2 * A
            print(f'{atom_temperature[0][0]:.2f} +{atom_temperature[0][1]-atom_temperature[0][0]} -{atom_temperature[0][0]-atom_temperature[0][2]} K and {atom_temperature[1][0]:.2f} +{atom_temperature[1][1]-atom_temperature[1][0]} -{atom_temperature[1][0]-atom_temperature[1][2]} K   {explain}')
